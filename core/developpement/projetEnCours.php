...
<div id="projetEnCours">
    <p style =" font-size: 40px; color: #ffebb6;"> DEVELOPPEMENT<span style =" font-size: 16px; color: #c8c8c8;">PROJETS EN COURS</span></p><hr style="margin-top: -32px;" />
</div>
    ...
    <legend> Liste des projets en cours</legend>
	 <table class="voteTop" style="border:none; text-align: center;">
		<thead>
			...
			<th width='80'><strong>Entrepris le</strong></th>
			<th width='140'><strong>Famille</strong></th>
			<th width='80'><strong>Avancement</strong></th>
		</thead>
<?php

		$sql= "SELECT * FROM `projets` WHERE idxstatutprojet = 1 ORDER BY `id` DESC LIMIT 0, 50" ;
		$req = $DB->prepare($sql);
		$req->execute();
